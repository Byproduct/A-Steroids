    ...
    public void process(AsteroidSpawner asteroidspawner) {
        ...
        ship.getShots().forEach(shot -> {
            asteroids.forEach(asteroid -> {
                if (shot.collide(asteroid) && (asteroid.isAlive()) && (shot.isAlive())) {
                    shot.setAlive(false);
                    ...
                    if (asteroid.getHealth() <= 0) {
                        ...
                        if (asteroid.getScale().equals("huge")) {
                            ...
                            for (int k = 0; k < newChunksX.size(); k++) {
                                for (int j = 0; j < 50; j++) {
                                    Spark spark = new Spark(newChunksX.get(k) + rnd.nextInt(100) - 50, newChunksY.get(k) + rnd.nextInt(100) - 50, "large");
                                    spark.setSpawnDelay(k * 3);
                                    queuedSparks.add(spark);
                                }
                            }
                            for (int n = 0; n < 300; n++) {                   // additional sparkplosions inside the asteroid area.
                                Spark spark = new Spark((int) (asteroid.getShape().getTranslateX()) + rnd.nextInt(300) - 150, (int) (asteroid.getShape().getTranslateY()) + rnd.nextInt(300) - 150, "huge");
                                spark.setSpawnDelay(2 + rnd.nextInt(15));
                                queuedSparks.add(0, spark);
                            }
                            for (int n = 0; n < 600; n++) {                  // additional sparkplosion at the heart of the ex-asteroid.
                                Spark spark = new Spark((int) (asteroid.getShape().getTranslateX()), (int) (asteroid.getShape().getTranslateY()), "huge");
                                spark.setSpawnDelay(2 + rnd.nextInt(15));
                                queuedSparks.add(0, spark);
                            }
                            for (int n = 0; n < 900; n++) {                  // additional sparkplosion at the impact location.
                                Spark spark = new Spark((int) (shot.getShape().getTranslateX()), (int) (shot.getShape().getTranslateY()), "huge");
                                spark.setSpawnDelay(2 + rnd.nextInt(15));
                                queuedSparks.add(0, spark);
                            }
                        }
                        ...
                        if (asteroid.getScale().equals("medium")) {
                            ...
                            for (int i = 0; i < 50; i++) {
                                Spark kipina = new Spark((int) (shot.getShape().getTranslateX()), (int) (shot.getShape().getTranslateY()), "medium");
                                queuedSparks.add(kipina);
                            }
                        }
                        if (asteroid.getScale().equals("small")) {
                            ...
                            for (int i = 0; i < 30; i++) {
                                Spark kipina = new Spark((int) (shot.getShape().getTranslateX()), (int) (shot.getShape().getTranslateY()), "small");
                                queuedSparks.add(0, kipina);
                            }
                        }
                    ...
                    if (asteroid.getHealth() >= 1) {
                        ...
                        for (int i = 0; i < 15; i++) {
                            Spark spark = new Spark((int) (shot.getShape().getTranslateX()), (int) (shot.getShape().getTranslateY()), "small");
                            queuedSparks.add(0, spark);
                        }
                    }
                }
